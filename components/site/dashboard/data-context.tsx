 ...
 */
const DashboardDataContext = createContext({ listings: LISTINGS, isReal: false });

export function DashboardDataProvider({ listings, children }) {
  const hasReal = Array.isArray(listings) && listings.length > 0;
  const value = { listings: hasReal ? listings : LISTINGS, isReal: hasReal };
  return (
