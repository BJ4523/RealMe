...
RealMe turns a real-estate agent into an on-camera AI presenter: photo + voice → HeyGen **avatar**; a **listing** (manual entry or paste-a-URL) → Claude-written script → HeyGen **walkthrough video**. Stack: Next.js 16 App Router (TS) · Supabase (auth + Postgres + Storage) · HeyGen · Anthropic · Tailwind/shadcn · Vercel.
...
- Tables: `profiles` (1:1 with `auth.users`), `avatars`, `listings`, `videos`. Private Storage buckets `avatar-sources`, `listing-photos`, `video-cache` use owner-prefixed paths (`<uid>/...`).
...
- **Listings** (`lib/listings/`): `ListingProvider` interface with `manual` and `url_scrape` (Firecrawl-powered, handles Zillow/Redfin/Realtor), registered in `index.ts`. UI/API depend only on the interface; concrete providers are resolved by `getListingProvider(id)`. MLS aggregators are explicitly deferred.
...
- Going-live steps and the design/feature status are in `docs/production.md` and `PRD_STATUS.md`. Deeper docs: `docs/architecture.md`, `docs/heygen.md`, `docs/setup.md`.
